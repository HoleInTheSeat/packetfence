...
sub create {
    ...
    my $use_connector = $item->{use_connector};
    ...
    if (!is_nt_hash_pattern($computer_password)) {
        ...
        for (my $i = 0; $i < @real_computer_names; $i++) {
            ...
            my ($add_status, $add_result) = pf::domain::add_computer(" ", $real_computer_name, $computer_password, $ad_server_ip, $ad_server_host, $dns_name, $workgroup, $ou, $bind_dn, $bind_pass, $use_connector);
            if ($add_status == $FALSE) {
                if ($add_result =~ /already exists(.+)use \-no\-add/) {
                    ($add_status, $add_result) = pf::domain::add_computer("-delete", $real_computer_name, $computer_password, $ad_server_ip, $ad_server_host, $dns_name, $workgroup, $ou, $bind_dn, $bind_pass, $use_connector);
                    if ($add_status == $FALSE) {
                    ...
                    ($add_status, $add_result) = pf::domain::add_computer(" ", $real_computer_name, $computer_password, $ad_server_ip, $ad_server_host, $dns_name, $workgroup, $ou, $bind_dn, $bind_pass, $use_connector);
                    if ($add_status == $FALSE) {
...
sub update {
    ...
    my $use_connector = $new_item->{use_connector};
    ...
    for (my $i = 0; $i < @real_computer_names; $i++) {
        ...
        if (!is_nt_hash_pattern($new_data->{machine_account_password})) {
            my ($add_status, $add_result) = pf::domain::add_computer("-delete", $real_computer_name, $computer_password, $ad_server_ip, $ad_server_host, $dns_name, $workgroup, $ou, $bind_dn, $bind_pass, $use_connector);
            if ($add_status == $FALSE) {
